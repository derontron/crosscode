...
git clone https://github.com/derontron/crosscode.git && cd crosscode && docker build -t crosscode/local:apache .

docker run -d -p 80:80 --name apache --link mysql:mysql -v /var/www/html:/var/www/html crosscode/local:apache
